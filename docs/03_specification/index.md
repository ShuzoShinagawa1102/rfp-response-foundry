# 提案書・RFP回答運用 / 03_specification
    ...
    ## 2. 境界づけられたコンテキスト
    - **Opportunity Intake**: 案件起票と評価
- **Question Decomposition**: 設問分解・担当割当
- **Knowledge Reuse**: 過去回答と証跡探索
- **Answer Assembly**: 回答生成・編集・出典管理
- **Review Governance**: 承認・差戻し
- **Submission & Learning**: 提出後の振返りと資産化
...
Opportunity "1" --> "0..1" RfpPackage : triggers
RfpPackage "1" --> "*" Question : contains
Question "1" --> "*" KnowledgeAsset : supported_by
Question "1" --> "0..1" DraftAnswer : proposed_as
DraftAnswer "1" --> "*" ReviewTask : reviewed_by
RfpPackage "1" --> "0..1" Submission : results_in
    ```

...
## 2. 推奨技術スタック
検索品質と共同編集が重要。Backend は TypeScript/NestJS、検索/再利用/文書比較は Python、ワークフローは Temporal、RDB は PostgreSQL、ナレッジ検索は OpenSearch または pgvector、本文差分と出典管理は object storage + metadata。共同編集は Google Docs / Office 連携か独自 rich-text モデルを選択。
